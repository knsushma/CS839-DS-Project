...
def extract_feature_set(words, entity_set):
    ...
    for index, entity in entity_set.items():
        ...
        if (is_index_not_out_of_bound(index, entity[1], corpus_length)):
            ...
            # F10
            feature_set.append(is_prev_word_verb(words[index - 1]))
            ...
            # F14
            feature_set.append(is_next_word_for_non_entity_recognition(words, index, words[index + 1]))

            # F15
            feature_set.append(is_next_word_for_non_entity_recognition_specifics(words[index + 1]))

        else:
            feature_set.extend([1,1,1,1,1,1,1])
...
entity_recognition_strong_prepositions = ["in", "at", "near", "to", "from", "across","inside", "outside",
                                           "on", "around"]
...
entity_recognition_weak_prepositions = ["as", "through", "throughout", "into", "leave", "like"]
...
entity_recognition_specifics = ["south", "north", "east", "west", "eastern" ,"western", "southern", "northern",
                                "continental", "central", "upper", "lower", "southeastern", "southwestern", "northeastern",
                                "northwestern", "mainland", "contemporary", "southeast", "southwest", "northeast", "northwest"]
...
non_entity_recognition_in_next_word = ["University", "Police", "government", "Times", "Post",
                                       ...
                                       "studio", "Corporation", "Entertainment", "Legislature", "Pictures",
                                       "Authority", "Province", "Hills", "Hill", "Awards", "Airport", "Talks",
                                       "Daily", "religion", "Brands", "territory", "Corporation"]
def is_next_word_for_non_entity_recognition(words, index, next):
    if (next == "Chelsea"):
        print(index, len(words), words[index+2])
    if ((next and any(entity == next for entity in non_entity_recognition_in_next_word))
        or (is_index_not_out_of_bound(index, 2, len(words)) and (any(entity == words[index+2] for entity in non_entity_recognition_in_next_word)))):
        return 0
    else:
        return 1

non_entity_recognition_in_prev_prev_word = ["University of", "College of", "Times of", "Gulf of", "Departments of",
                                            "Department of", "Secretary of", "friend of", "Union on", "State University",
                                            "Academy of", "Art of", "Republic of"]
