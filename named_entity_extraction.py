...
disposable_words = ['a', 'an', 'the', 'have', 'has', 'been', 'was', 'is', 'by', 'to', 'at', 'for', 'in', 'of', 'from',
                    'like', 'with', 'were',
                    'are', 'what', 'where', 'how', 'why', 'who', 'it', "it's", 'and', 'but', 'on', "its", 'we', 'our',
                    'over',
...
for file_prefix in range(101, 230):
    ...
    for index, word in iter_words:
        if "location" in word:
            ...
            if word and word[0].isupper() and word.lower() not in disposable_words and not (
    ...
    dataset = form_dataset_matrix(positive_entity_feature_set, negative_entity_feature_set, named_entity,
                                  unnamed_entity)
